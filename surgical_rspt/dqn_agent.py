...
class Agent(object):
    ...
    def __init__(self, state_size, action_size, feature_dim, bee_index, brain_name, arguments):
        self.state_size = state_size # 42
        self.action_size = action_size # 2
        self.feature_dim = feature_dim # 2048
        self.bee_index = bee_index
        self.learning_rate = arguments['learning_rate']
        self.gamma = 0.95
        self.brain = Brain(self.feature_dim, self.action_size, brain_name, arguments)
    ...
    def greedy_actor(self, state):
        if np.random.rand() <= self.epsilon:
            ...
            return np.argmax(self.brain.predict_one_sample(state).detach().numpy())
    ...
    def find_targets_per(self, batch):
        ...
        p = self.brain.predict(states) # 현재 state에서 main model의 Q 값
        p_ = self.brain.predict(states_) # 다음 state에서 main model의 Q 값
        pTarget_ = self.brain.predict(states_, target=True) # 다음 state에서 target model의 Q값
        
        # torch to numpy
        p, p_, pTarget_ = p.detach().numpy(), p_.detach().numpy(), pTarget_.detach().numpy()

        x = np.zeros((batch_len, self.state_size, self.feature_dim))
